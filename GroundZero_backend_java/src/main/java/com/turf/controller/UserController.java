...
//	}
	@PostMapping("/signin")
	public ResponseEntity<?> userLogIn(@RequestBody SignInRequest dto) throws NotFoundException{
		try {
	...
	public ResponseEntity<?> getById(@PathVariable @Valid Long userId){
		try {
			UserRespDTO respDto=userService.getById(userId);
			return ResponseEntity.ok(respDto);
		}
		catch (Exception e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
	...
	@PutMapping("/{userId}")
	public ResponseEntity<?> updateUser(@PathVariable @Valid Long userId,@RequestBody UserUpdateDTO dto){
		try {
			UserRespDTO resp=userService.updateUser(userId,dto);
			return ResponseEntity.ok(resp);
		}
		catch(Exception e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
	...
	public ResponseEntity<?> changePass(@RequestBody ChangePasswordDTO dto) throws NotFoundException{
		try {
			ApiResponse respDto=userService.changePass(dto);
			return ResponseEntity.ok(respDto);
		}
		catch(RuntimeException e) {
			System.out.println(e);
